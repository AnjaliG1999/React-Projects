...
class Search extends Component {
    ...
    componentDidUpdate = () => {
        ...
        if (this.text != this.state.query) {
            var url = "https://apibyashu.herokuapp.com/api/searchcakes?q=" + encodeURIComponent(this.text)
            axios({
                url: url,
                method: "get"
            }).then((response) => {
    ...
    filterByWeight = (event) => {
        ...
        var results = this.state.searchResults.filter(cake => {return cake.weight == weight});
        console.log(results);
        this.setState({searchResults: results})
    }
    render() {
        return (
            <div className="search" id="Search">
                ...
                <div className="container">
                    {this.state.searchResults && this.state.searchResults.length > 0 && <div>
                        <h1 style={{ textAlign: "center" }}>Results based on your Search</h1>
                        <div className="row">
                            <div className="col-3">
                                ...
                                <div className="searchCategory">
                                    ...
                                    <div className="filterSearch">
                                        <button data-bs-toggle="collapse" data-bs-target="#collapseWeight" aria-expanded="false" aria-controls="collapseWeight">
                                            <b>Weight</b>
                                        </button>
                                        <ul class="collapse" id="collapseWeight" style={{listStyle:"none", textAlign:"left"}}>
                                    ...
                                    <div className="filterSearch">
                                        <button data-bs-toggle="collapse" data-bs-target="#collapsePrice" aria-expanded="false" aria-controls="collapsePrice">
                                            <b>Price</b>
                                        </button>
                                        <ul class="collapse" id="collapsePrice" style={{listStyle:"none", textAlign:"left"}}>
                                            <li>Under Rs. 299</li>
                                            <li>Rs.300-Rs.499</li>
                            ...
                            <div className="col-9">
                                {
                                    this.state.searchResults.map(item => {
                                        return <CartItem cakesData={item} fromLocation={"search"} />
                                    })
                                }
                            </div>
                        </div>
                    ...
                    <div>
                        {
                            (!this.state.searchResults.length > 0) && <h2>No results found, please search for another cake type</h2>
                        }
                    </div>
                </div>
            </div>
